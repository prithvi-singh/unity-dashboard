    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06),0_6px_24px_rgba(0,0,0,0.04)] overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 text-[10px] text-gray-400">
          ...
          <span className="w-4 h-4 rounded bg-sky-100" />
          <span className="w-4 h-4 rounded bg-sky-400" />
          <span className="w-4 h-4 rounded bg-violet-600" />
          <span className="px-2 py-0.5 rounded bg-gray-50">More</span>
        </div>
      </div>
      <div className="overflow-x-auto scrollbar-thin">
        <table className="text-xs border-separate border-spacing-0.5 p-3 w-full" aria-label="Top user activity heatmap">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-white pr-3 pb-1 text-left font-bold text-[10px] text-gray-400 uppercase tracking-[0.08em] min-w-[140px]">
                User
              </th>
              {dates.map((d) => (
                <th key={d} className="text-center font-normal text-gray-400 pb-1 w-9 min-w-[2.25rem]" title={d}>
                  {mmdd(d)}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {topUsers.map((user) => {
              ...
              return (
                <tr key={user.id}>
                  <td className="sticky left-0 z-10 bg-white pr-3 py-0.5 text-gray-700 font-medium whitespace-nowrap">
                  ...
                  {dates.map((d) => {
                    ...
                    return (
                      <td
                        ...
                        className={`w-9 h-7 text-center rounded align-middle font-medium tabular-nums ${cellBg(count)}`}
                      >
                        {count > 0 ? count : ''}
                      </td>
                    );
